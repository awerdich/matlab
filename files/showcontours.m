...
if cv==1 && showframe==1
    ...
    if askpolygon==1
        fprintf('Create closed polygon. Double-mouse-click when finished.')
        axes(isoczoom);
        ...
        if isempty(POLYGONPOSITIONS)==1
            fprintf('Empty polygon positions. Stopping execution.')
            return
        end
    end
end
...
n_signal_pixels=sum(SIGNALFRAMEINTERP_0(:));
...
if askpolygon==1
    ...
    X=(POLYGONPOSITIONS(:,1)-FrameXLim(1))/pixelinterval;
    Y=(POLYGONPOSITIONS(:,2)-FrameYLim(1))/pixelinterval;
    M=size(POLYROI,1);N=size(POLYROI,2);
    POLYROI=poly2mask(X,Y,M,N);
    % Create a new SIGNAL mask
    XS=POLYGONPOSITIONS(:,1);
    YS=POLYGONPOSITIONS(:,2);
    MS=size(SIGNAL,1);NS=size(SIGNAL,2);
    SIGNAL_POLY=poly2mask(XS,YS,MS,NS);
end
%% modify SIGNALFRAMEINTERP)=_0 for if there is a POLYROI
if askpolygon==1 && isempty(POLYROI)==0
    fprintf('POLYROI detected. Modified SIGNAL mask.\n')
    polymask=POLYROI;
    polymask(SIGNALFRAMEINTERP_0==0)=0;
    SIGNALFRAMEINTERP_0=polymask;
    new_signal_pixels=sum(SIGNALFRAMEINTERP_0(:));
    fprintf(['SIGNAL area reduced to ', num2str(new_signal_pixels/n_signal_pixels*100), ' pct.\n'])
end
