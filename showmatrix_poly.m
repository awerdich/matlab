...
use_poly = 1; %1=use a polygon as ROI (only for APD currently)
if use_poly==1
    showframe=0;
end

setframeumwidth=40;%frame unit length [um]. If zero, click corners of frame
setframeumheight=40;
...
% Define coordinates of pixels within ROI
ROI_coordinates=[];
if use_poly==1
    axes(mapaxes) 
    h = drawpolygon(mapaxes);
    pos = h.Position;
    pos = round(pos/resizefactor);
    % determine the points within and on the polygon
    for i=1:size(C,1)
        for j=1:size(C,2)
            [in, on] = inpolygon(j, i, pos(:,1), pos(:,2));
            if in==1 | on==1
                ROI_coordinates=[ROI_coordinates;[i, j]];
            end
        end
    end
else
    % get the ROI_coordinates for rectangular ROI
...
%% define the ROI_coodinates
%get mean transient
if limits<5
    T=[];A=[];B=[];FRAMEij=[];LOW=[];HIGH=[];AMP=[];totalscans=300;U=[];KS=[];DURij=[];
    totalscans=2000;
    threshold=0.5;
    for row=1:length(ROI_coordinates)
        i = ROI_coordinates(row, 1);
        j = ROI_coordinates(row, 2);
